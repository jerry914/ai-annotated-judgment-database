      ...
      <div class="search-main">
        ...
        <div class="advanced-section">
          <div class="adv-field">
            ...
            <input v-model="advFilters.basicInfo" class="adv-input" />
          </div>
          <div class="adv-field">
            <label class="adv-label">主文中的關鍵字</label>
            <input v-model="advFilters.syllabusKeyword" class="adv-input" />
          </div>
          <div class="adv-field">
            <label class="adv-label">見解關鍵字</label>
            <input v-model="advFilters.opinionKeyword" class="adv-input" placeholder="輸入見解關鍵字..." />
          </div>
        ...
        <div v-if="results.length > 0" class="download-row">
          <button class="btn-download" @click="downloadCSV">下載見解相關資料</button>
        </div>
      </div>
    </div>
...
export default {
  ...
  data() {
    return {
      filters: {},
      advFilters: { basicInfo: '', syllabusKeyword: '', opinionKeyword: '' },
      results: [],
      loading: false,
      searched: false,
      page: 1,
  ...
  methods: {
    ...
    async doSearch() {
      ...
      try {
        ...
        if (this.advFilters.opinionKeyword) params.prediction = this.advFilters.opinionKeyword;
        ...
        const resp = await axios.get('/api/criminal/opinion', { params });
        const data = resp.data;
        if (data.opinion) {
          const raw = data.opinion.data || [];
          if (raw.length > 0 && raw[0].group) {
            this.results = raw.flatMap(item => item.group);
          } else {
            this.results = raw;
          }
          this.total = data.opinion.meta?.total || 0;
          this.totalPages = data.opinion.meta?.total_pages || 0;
        } else {
          this.results = [];
          this.total = 0;
        }
    ...
    resetFilters() {
      this.filters = {};
      this.advFilters = { basicInfo: '', syllabusKeyword: '', opinionKeyword: '' };
      this.results = [];
      this.searched = false;
      this.page = 1;
      this.fetchFilteredStats();
    ...
    downloadCSV() {
      ...
      const rows = this.results.map((r, i) =>
        [i + 1, `"${r.case_num || ''}"`, r.jud_date, `"${r.case_type || ''}"`, `"${(r.syllabus || '').replace(/"/g,'""')}"`, `"${(r.opinion || r.sentence || '').replace(/"/g,'""')}"`].join(',')
      ).join('\n');
      const blob = new Blob(['\ufeff' + header + rows], { type: 'text/csv;charset=utf-8' });
      const url = URL.createObjectURL(blob);
      const a = document.createElement('a');
      a.href = url;
      a.download = '見解檢索結果.csv';
      a.click();
...
.page-title { font-family: var(--font-display); font-size: 1.5rem; font-weight: 700; margin: 0 0 1.25rem 0; }
.advanced-section {
...
@media (max-width: 900px) { .advanced-section { grid-template-columns: 1fr; } }
.adv-label { display: block; font-size: 12px; font-weight: 500; color: var(--color-muted); margin-bottom: 0.35rem; }
.adv-input { width: 100%; padding: 8px 12px; border: 1px solid rgba(0,0,0,0.1); border-radius: var(--radius-sm); font-size: 14px; box-sizing: border-box; }
.adv-input:focus { outline: none; border-color: var(--color-primary); }
.btn-advanced { padding: 8px 20px; background: var(--color-primary); color: #fff; border: none; border-radius: var(--radius-sm); font-size: 14px; font-weight: 600; cursor: pointer; white-space: nowrap; }
.results-section { background: var(--color-surface); border-radius: var(--radius-lg); box-shadow: var(--shadow-soft); overflow: hidden; }
.results-table { width: 100%; border-collapse: collapse; font-size: 13px; }
.results-table th { background: var(--color-primary); color: #fff; padding: 10px 12px; text-align: left; font-weight: 600; font-size: 12px; }
.results-table td { padding: 10px 12px; border-bottom: 1px solid rgba(0,0,0,0.05); vertical-align: top; }
...
.pagination { display: flex; justify-content: center; align-items: center; gap: 1rem; padding: 1rem; font-size: 13px; }
.pagination button { padding: 6px 16px; border: 1px solid rgba(0,0,0,0.12); border-radius: var(--radius-sm); background: var(--color-surface); cursor: pointer; font-size: 13px; }
.pagination button:disabled { opacity: 0.4; cursor: default; }
.download-row { margin-top: 1rem; text-align: center; }
.btn-download { padding: 10px 28px; background: var(--color-primary); color: #fff; border: none; border-radius: var(--radius-default); font-size: 14px; font-weight: 600; cursor: pointer; }
...
.dialog-box { background: var(--color-surface); border-radius: var(--radius-lg); max-width: 700px; width: 90%; max-height: 80vh; overflow-y: auto; box-shadow: var(--shadow-lifted); }
.dialog-header { display: flex; justify-content: space-between; align-items: center; padding: 1.25rem 1.5rem; border-bottom: 1px solid rgba(0,0,0,0.08); }
